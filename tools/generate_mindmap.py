...
async def generate_mindmap(
  ...
  include_sources: bool = True
    ...
    try:
        ...
        context_prompt = ""
        if context:
            context_prompt = f"Use this context from the user's documents:\n{context}\n\n"
        
        # Create prompt for mind map generation
        prompt = f"""Generate a hierarchical mind map for the topic: {topic}

{context_prompt}Create a structured JSON mind map with the following format (hierarchical tree):
{{
  "id": "root",
